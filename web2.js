...
function showsearch() {
  ...
  searchBtn.className = "fa fa-times"; /* thot x*/
  searchform.style.display = "flex"; /*tftah l search */
  searchBtn.setAttribute("onclick", "hiddensearch()"); /* t3ayet lal fun l btsaker l search */
...
function hiddensearch() {
  ...
  searchBtn.className = "fa fa-search"; /* bas tsaker bthawel l x la search */
  searchform.style.display = "none"; /* tsaker l search */
  searchBtn.setAttribute("onclick", "showsearch()"); /* t3ayet lal fun l bteftah l search */
...
function showcart() {
  ...
  /*btse3dna bel sfha el kbire bel hone laa*/
...
window.onload = function () {
  ...
  if (document.getElementsByClassName("buy").length != 0) {
    ...
    document.getElementsByClassName("buy")[0].addEventListener("click", buybuttonclick);
  ...
  for (i = 0; i < staricons.length; i++) {
    for (j = 1; j <= 5; j++) {
      ...
      if (staricon) {
        staricon.addEventListener("click", function () { toggleStar(this); });
      }
      else {
        console.log("errorrr")
      }
...
function addcartclicked(event) {
  /*addcartclicked is likely the name of a function in a JavaScript program that is triggered when a user clicks on an "Add to Cart" button on a web page.*/
  ...
  for (let i = 0; i < quantityinputs.length; i++) {
    ...
    input.addEventListener("change", function (event) { quantitychange(event, available); });
  }
  updatetotal()
}
...
function addproducttocart(title, price, productImg) {
  ...
  let shopnotification = setInterval(() => { carticon.classList.add('active') }, 0);
  setTimeout(() => { clearInterval(shopnotification); carticon.classList.remove('active') }, 1000);
